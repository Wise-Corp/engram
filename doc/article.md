...
## Two Kinds of Signal: Static and Dynamic

The extraction layer described so far deals with **static signals** — the state of the codebase, git history, dependencies, conventions. These tell the AI what the project looks like. But they say nothing about what the project means, or about the human building it.

The second category is **dynamic signals** — derived not from the codebase, but from the session transcript itself. These split further into two dimensions that serve fundamentally different purposes.

### Dimension A: User Preferences

Every interaction between a developer and an AI coding agent contains implicit preference data:

- **Acceptance patterns**: When Claude proposes code and the developer says "do it" versus "no, actually..." — that's a labeled training signal, sitting in plain text. A shell script can classify user responses into accepted, rejected, and modified using simple keyword matching. No AI required.
- **Correction vocabulary**: When a developer pushes back, they often state preferences explicitly — "too verbose", "split this up", "don't use classes here", "I prefer functions." These are extractable with grep and are among the most valuable signals in the entire system.
- **Iteration depth**: If the same file is edited four times before the developer accepts the result, that's a measurable friction signal. It reveals mismatches between the AI's defaults and the developer's expectations.

### Dimension B: Domain Knowledge

This is arguably more valuable than user preferences and almost entirely absent from current memory systems. Most real projects implement a business domain — an invoicing system, a logistics platform, a clinical workflow — with its own terminology, entity relationships, state machines, and hard constraints. Claude arrives at each session knowing TypeScript and Python but not knowing that a Quotation is not an Order, or that a shipment cannot exist without a validated delivery note.

The session transcript is full of domain corrections that are deterministically extractable:

- **Terminology corrections**: "That's not a client, it's a vendor — they're different entities here." Detectable by patterns like "X is not Y", "when I say X I mean Y", "don't confuse X with Y."
- **Business rule statements**: "We can't hard-delete invoices because of audit compliance." The word "because" is a strong signal that the user is explaining a domain constraint, not a code preference.
- **Workflow violations**: "No, you can't skip the validation step — the order has to go through approval first." These contain explicit state-ordering constraints that should become permanent memory.
- **Entity relationship clarifications**: "A client can have multiple contacts, but a contact belongs to exactly one client." These are data model facts that Claude keeps getting wrong until someone writes them down.
- **Repeated explanations**: If the user writes a 100-word paragraph explaining how the billing cycle works — that's a domain lesson. If they write it twice across sessions, the memory system has failed at its most basic job.

All of these are extractable from JSONL transcripts using keyword matching, sentence-length heuristics, and noun frequency analysis. They cost nothing to compute. And they capture something that no amount of codebase scanning will reveal: the business logic that exists in the developer's head but not yet fully in the code.

### The Combined Effect

When both dimensions feed into the synthesis step, the resulting memory document doesn't just describe the project or the developer — it describes the collaboration in its full context. A future session starts with Claude knowing not only that the project uses FastAPI with Jinja templates (static signals), and that the developer prefers small commits and functional patterns (user preferences), but also that a Quotation becomes an Order only after client approval, that invoices require soft-delete, and that the payment workflow has five mandatory states that cannot be skipped (domain knowledge).

That's the difference between an AI that can read the codebase and an AI that understands the business.
